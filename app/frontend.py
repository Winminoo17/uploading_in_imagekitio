...
def create_transformed_url(original_url, transformation_params, caption=None):
    ...
    if not parsed_url.scheme or not parsed_url.netloc or not parsed_url.path.strip("/"):
        return original_url
    ...
    transformed_path = f"/{path_parts[0]}/tr:{transformation_params}/{path_parts[1]}"
    return parsed_url._replace(path=transformed_path).geturl()
    ...
    if response.status_code == 200:
        ...
        for post in posts:
            ...
            with col2:
                if post.get('is_owner', False):
                    if st.button("🗑️", key=f"delete_{post['id']}", help="Delete post"):
                        # Delete the post
                        response = requests.delete(f"http://localhost:8000/delete/{post['id']}", headers=get_headers())
                        if response.status_code == 200:
            ...
            if post['file_type'] == 'image':
                ...
                # For videos: specify only height to maintain aspect ratio.
